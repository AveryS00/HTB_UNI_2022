...
class Wizard:
    ...
    def __init__(self):
        # This is a 127 bit long value, randomly picked, not prime
        # This is a Truncated LCG
    ...
    def dontAcceptDefeat(self, playerHealth, flag):
        ...
        finalSpellIngredient = str(self.spell >> 95).encode()
        test = hashlib.md5(finalSpellIngredient)
        finalSpellIngredient = test.digest()
        ...
        disruptedFlag = finalSpell.encrypt(
            pad("You're a wizard Harry, ".encode() + disruptedFlag.encode(),
                AES.block_size)).hex()
        # print(disruptedFlag)
        # print(pad("You're a wizard Harry, ".encode() + disruptedFlag.encode(), AES.block_size))
        #
        # new_decryptor = AES.new(finalSpellIngredient, AES.MODE_CBC)
        # print(new_decryptor.decrypt(bytes.fromhex(disruptedFlag)))
    ...
    try:
        ...
        if playerHealth <= 0:
            ...
            disruptedFlag, scrollOfWorthiness = encryptionWizard.dontAcceptDefeat(
                playerHealth, 'success-0r_m3$beeThatwas&J!^tfall')
